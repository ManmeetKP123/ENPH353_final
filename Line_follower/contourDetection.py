...
class image_converter:
    ...
    def callback(self, data):
        ...
        parkingIDs = [2, 8, 3, 7, 4, 5, 6, 1] #added 7 because it keeps sending the garbage values
        ...
        if len(contours) >= 2:
            ...
            if smaller_contour_area > 2800:
                ...
                col_idx = col_idx[np.where(col_idx > 750)]
                col_idx = col_idx[np.where(col_idx < (len(sum)-filter_threshold))]
                print("shape of col index " + str(col_idx.shape))
                ...
                try:
                    ...
                    lower_hsv_plates_light = np.array([119, 9, 0])
                    upper_hsv_plates_light = np.array([119, 255, 255])

                    lower_hsv_plates_dark = np.array([101, 67, 0])
                    upper_hsv_plates_dark = np.array([125, 83, 153])

                    hsv_ranges_plates = [[lower_hsv_plates_light, upper_hsv_plates_light], 
                                        [lower_hsv_plates_dark, upper_hsv_plates_dark]]

                    masked_img_plates = 0

                    for lower, upper in hsv_ranges_plates:
                        pre_mask_img_copy = pre_mask_img.copy()
                        masked_img_plates += cv2.inRange(pre_mask_img_copy, lower, upper)
                    print("made it here")
                    

                    print(sum(masked_img_plates))
                    # if (sum(masked_img_plates) > 10000):
                    cv2.imshow("Masked Image", masked_img_plates)
                ...
                # cv2.waitKey(2)
                    ...
                    # contours, hierarchy = cv2.findContours(image=masked_img_plates, mode=cv2.RETR_EXTERNAL, 
                    #     method=cv2.CHAIN_APPROX_SIMPLE)[-2:]
        ...
        if (self.predict != None and self.plate_number < len(parkingIDs)):
            ...
            if (len(self.list_of_plates) > 5):
                ...
                if (diff > 2 and diff < 3):
                    ...
                    print("made it here")
                    print("time difference " + str(rospy.get_time() - self.plate_detected))
                    self.predict = None
                    self.plate_number += 1

                    if (self.plate_number >= len(parkingIDs)):
                        self.plate_number = 0
